...
public class Calculator_v {
	...
	static boolean isCorrectExpression(String text) {
		...
		String operators = "/*-+^()";
		boolean startWith = false, endWith = false;
		boolean isTrue = false;
		
		for(int i = 0; i < operators.length(); i++) {
			startWith = text.startsWith(operators.charAt(i) + "");
			endWith = text.endsWith(operators.charAt(i) + "");
			if(startWith || endWith) {
				break;
			}
		}
		if(isCorrectBaracket(text) && !(startWith && endWith)) {
			for(int i = 1; i < text.length() - 1; i++) {
				if(operators.contains(text.charAt(i) + "")) {
					isTrue = !(expectedOperator(text.charAt(i + 1)) && expectedOperator(text.charAt(i - 1)));
				}
			}
		}
		return isTrue;
	...
	static boolean isCorrectBaracket(String str) {
		boolean result = false;
		String[] operatorsSplit = new String[] {"-","\\+","/","\\*","x","\\^","X"};
		String operatorRegex = String.join("|", operatorsSplit);
		str = str.replaceAll("[0-9]", "").replaceAll(operatorRegex, "");
		
		int baracket1Open = stringCharCount(str, '(');
		int baracket1Close = stringCharCount(str, ')');
		int bracketOpen1 = 0, baracketClose1 = 0;

		if (baracket1Open == baracket1Close) {
			...
			if (baracket1Open == bracketOpen1 && baracket1Open == baracketClose1) {
				result = true;
			} 
		}
		return result;
	...
	static int stringCharCount(String text, char character) {
		int count = 0;
		for (char c : text.toCharArray()) {
			if (c == character) {
				count++;
